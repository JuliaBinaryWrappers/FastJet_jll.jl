# Autogenerated wrapper script for FastJet_jll for x86_64-w64-mingw32-cxx11
...
const libNsubjettiness_splitpath = ["bin", "libNsubjettiness.dll"]
...
const libNsubjettiness = "libNsubjettiness.dll"


# Relative path to `libLundPlane`
const libLundPlane_splitpath = ["bin", "libLundPlane.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libLundPlane = "libLundPlane.dll"


# Relative path to `libClusteringVetoPlugin`
const libClusteringVetoPlugin_splitpath = ["bin", "libClusteringVetoPlugin.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libClusteringVetoPlugin = "libClusteringVetoPlugin.dll"


# Relative path to `libScJet`
const libScJet_splitpath = ["bin", "libScJet.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libScJet = "libScJet.dll"


# Relative path to `libsiscone_spherical`
const libsiscone_spherical_splitpath = ["bin", "libsiscone_spherical-0.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libsiscone_spherical = "libsiscone_spherical-0.dll"


# Relative path to `libSoftKiller`
const libSoftKiller_splitpath = ["bin", "libSoftKiller.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libSoftKiller = "libSoftKiller.dll"


# Relative path to `libQCDAwarePlugin`
const libQCDAwarePlugin_splitpath = ["bin", "libQCDAwarePlugin.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libQCDAwarePlugin = "libQCDAwarePlugin.dll"


# Relative path to `libfastjetplugins`
const libfastjetplugins_splitpath = ["bin", "libfastjetplugins-0.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libfastjetplugins = "libfastjetplugins-0.dll"


# Relative path to `libJetFFMoments`
const libJetFFMoments_splitpath = ["bin", "libJetFFMoments.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libJetFFMoments = "libJetFFMoments.dll"


# Relative path to `libSubjetCounting`
const libSubjetCounting_splitpath = ["bin", "libSubjetCounting.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libSubjetCounting = "libSubjetCounting.dll"


# Relative path to `libEnergyCorrelator`
const libEnergyCorrelator_splitpath = ["bin", "libEnergyCorrelator.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libEnergyCorrelator = "libEnergyCorrelator.dll"


# Relative path to `libJetCleanser`
const libJetCleanser_splitpath = ["bin", "libJetCleanser.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libJetCleanser = "libJetCleanser.dll"


# Relative path to `libfastjettools`
const libfastjettools_splitpath = ["bin", "libfastjettools-0.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libfastjettools = "libfastjettools-0.dll"


# Relative path to `libValenciaPlugin`
const libValenciaPlugin_splitpath = ["bin", "libValenciaPlugin.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libValenciaPlugin = "libValenciaPlugin.dll"


# Relative path to `libJetsWithoutJets`
const libJetsWithoutJets_splitpath = ["bin", "libJetsWithoutJets.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libJetsWithoutJets = "libJetsWithoutJets.dll"


# Relative path to `libRecursiveTools`
const libRecursiveTools_splitpath = ["bin", "libRecursiveTools.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libRecursiveTools = "libRecursiveTools.dll"


# Relative path to `libClusteringVetoPlugin`
const libClusteringVetoPlugin_splitpath = ["bin", "libConstituentSubtractor.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libClusteringVetoPlugin = "libConstituentSubtractor.dll"


# Relative path to `libfastjet`
const libfastjet_splitpath = ["bin", "libfastjet-0.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libfastjet = "libfastjet-0.dll"


# Relative path to `libsiscone`
const libsiscone_splitpath = ["bin", "libsiscone-0.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libsiscone = "libsiscone-0.dll"


# Relative path to `libFlavorCone`
const libFlavorCone_splitpath = ["bin", "libFlavorCone.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libFlavorCone = "libFlavorCone.dll"


# Relative path to `libGenericSubtractor`
const libGenericSubtractor_splitpath = ["bin", "libGenericSubtractor.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libGenericSubtractor = "libGenericSubtractor.dll"


# Relative path to `libVariableR`
const libVariableR_splitpath = ["bin", "libVariableR.dll"]
...
# This must be `const` so that we can use it with `ccall()`
const libVariableR = "libVariableR.dll"
