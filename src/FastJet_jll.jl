...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import FastJet_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("FastJet_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "FastJet_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"FastJet", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "FastJet_jll", "src", "FastJet_jll.jl"))
        @info("FastJet_ll dev'ed out to /depot/dev/FastJet_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("7111ce8a-33e8-59c0-b870-b64149b8fb4c"))
